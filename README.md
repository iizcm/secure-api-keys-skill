...
cp -r devops/secure-api-keys ~/.hermes/skills/devops/secure-api-keys/

# Or install directly from SKILL.md URL (if your runtime supports it)
<agent-cli> skills install https://raw.githubusercontent.com/iizcm/secure-api-keys-skill/main/SKILL.md
```
...
```text
skill_view(name="devops/secure-api-keys")
```
...
```text
Use the secure-api-keys skill to do its primary task.
```
